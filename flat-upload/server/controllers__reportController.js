...
// Função auxiliar para obter data no fuso horário do Brasil
const getBrasilDate = () => {
  const now = new Date();
  return new Date(now.getFullYear(), now.getMonth(), now.getDate());
};

// Função para criar data de início (00:00:00) no horário local
const getLocalStartDate = (date) => {
  if (!date) return getBrasilDate();
  
  const d = new Date(date);
  return new Date(d.getFullYear(), d.getMonth(), d.getDate());
};

// Função para criar data de fim (23:59:59) no horário local
const getLocalEndDate = (date) => {
  if (!date) {
    const end = getBrasilDate();
...
exports.salesReport = async (req, res) => {
  try {
    ...
    startDate = getLocalStartDate(startDate);
    endDate = getLocalEndDate(endDate);
    
    console.log(`Gerando relatório de vendas para o período: ${startDate.toISOString()} até ${endDate.toISOString()}`);
...
exports.topProductsReport = async (req, res) => {
  try {
    ...
    console.log(`Generating top products report for period: ${startDate.toISOString()} to ${endDate.toISOString()}`);
    ...
    const topProductsAggregation = await OrderItem.aggregate([
      ...
      {
        $match: {
          'orderInfo.createdAt': { $gte: startDate, $lte: endDate },
...
exports.dailySalesReport = async (req, res) => {
  try {
    ...
    const startDate = getBrasilDate();
    startDate.setDate(startDate.getDate() - days);
    
    console.log(`Generating daily sales report for last ${days} days`);
    ...
    console.log(`Daily sales aggregation found data for ${dailySalesAggregation.length} days`);
...
exports.waiterPerformanceReport = async (req, res) => {
  try {
    ...
    console.log(`Generating waiter performance report for period: ${startDate.toISOString()} to ${endDate.toISOString()}`);
    ...
    console.log(`Waiter aggregation found performance data for ${waiterAggregation.length} waiters`);
