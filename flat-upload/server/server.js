...
// Middleware para aplicações que usam datas
app.use((req, res, next) => {
  // Se a requisição tiver query params de data
  if (req.query.startDate) {
    const startDate = new Date(req.query.startDate);
    // Ajustar para o início do dia local
    req.query.startDate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate(), 0, 0, 0).toISOString();
  }
  
  if (req.query.endDate) {
    const endDate = new Date(req.query.endDate);
    // Ajustar para o final do dia local
    req.query.endDate = new Date(endDate.getFullYear(), endDate.getMonth(), endDate.getDate(), 23, 59, 59, 999).toISOString();
  }
