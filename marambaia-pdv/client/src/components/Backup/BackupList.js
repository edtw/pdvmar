...
const BackupList = ({ onRefresh }) => {
  ...
  const handleDownload = async (backupId) => {
    try {
      ...
      window.open(
        `${process.env.APP_API_URL}/backups/${backupId}/download`,
        "_blank"
      );
    } catch (error) {
    ...
    try {
      ...
      if (response.data.success) {
        toast({
          ...
          description:
            "A lista de backups foi sincronizada com os arquivos do servidor",
          status: "success",
          duration: 3000,
          isClosable: true,
        });
    ...
    try {
      ...
      if (response.data.success) {
        toast({
          ...
          description:
            "O backup foi restaurado com sucesso. A página será recarregada.",
          status: "success",
          duration: 5000,
          isClosable: true,
        });
      ...
      toast({
        title: "Erro",
        description:
          error.response?.data?.message || "Erro ao restaurar backup",
        status: "error",
        duration: 3000,
        isClosable: true,
      });
  ...
  if (backups.length === 0) {
    return (
      <Alert status="info" borderRadius="md">
        <AlertIcon />
        <Text>
          Nenhum backup encontrado. Clique em "Fazer Backup" para criar o
          primeiro backup.
        </Text>
      </Alert>
    );
