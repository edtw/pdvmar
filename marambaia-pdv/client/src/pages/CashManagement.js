...
const CashManagement = () => {
  ...
  const fetchTransactions = useCallback(async () => {
    console.log('🟢 fetchTransactions: Iniciando busca de transações');
    console.log('🟢 fetchTransactions: selectedCashRegister:', selectedCashRegister);
    console.log('🟢 fetchTransactions: fetchTransactionsRef.current:', fetchTransactionsRef.current);
    
    if (!selectedCashRegister) {
      console.log('⚠️ fetchTransactions: Nenhum caixa selecionado, abortando');
      return;
    }
    
    // Evitar múltiplas requisições simultâneas
    if (fetchTransactionsRef.current) {
      console.log('⚠️ fetchTransactions: Requisição já em andamento, abortando');
      return;
    }
    ...
    try {
      ...
      // Verificar se o componente ainda está montado
      if (!isMountedRef.current) {
        console.log('⚠️ fetchTransactions: Componente desmontado, abortando');
        return;
      }
      ...
      const url = `/cash-registers/${selectedCashRegister._id}/transactions?startDate=${startDateStr}&endDate=${endDateStr}`;
      console.log('🟢 fetchTransactions: Chamando API:', url);
      
      const response = await api.get(url);
      console.log('🟢 fetchTransactions: Resposta da API:', response.data);
      
      // Verificar novamente se o componente ainda está montado
      if (!isMountedRef.current) {
        console.log('⚠️ fetchTransactions: Componente desmontado após resposta, abortando');
        return;
      }
      
      if (response.data.success) {
        console.log('✅ fetchTransactions: Sucesso, atualizando transações');
        setTransactions(response.data.transactions);
      }
    } catch (error) {
      console.error('❌ fetchTransactions: Erro ao buscar transações:', error);
      
      // Verificar se o erro é de aborto de requisição
      if (error.code === 'ECONNABORTED' || error.name === 'CanceledError') {
        console.log('⚠️ fetchTransactions: Requisição cancelada');
        return;
      }
      ...
      console.log('🟢 fetchTransactions: Finalizando, setando transactionsLoading false');
      if (isMountedRef.current) {
