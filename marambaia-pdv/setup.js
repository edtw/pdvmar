...
console.log('==========================================================');
console.log(`Platform detected: ${isWindows ? 'Windows' : (isLinux ? 'Linux' : (isMac ? 'macOS' : 'Unknown'))}`);
console.log('\n');
...
async function setup() {
  try {
    ...
    console.log('You can now start the application with "npm run dev" in both server and client directories.');
  ...
  try {
    if (isWindows) {
      ...
      try {
        ...
        console.log('1. Download and install MongoDB Community Server: https://www.mongodb.com/try/download/community');
        console.log('2. Use MongoDB Atlas (cloud): https://www.mongodb.com/cloud/atlas/register');
        
        const answer = await askQuestion('Would you like to use MongoDB Atlas instead of local installation? (y/n): ');
        if (answer.toLowerCase() === 'y') {
          const uri = await askQuestion('Please enter your MongoDB Atlas connection string: ');
          mongoUri = uri;
          return;
        } else {
      ...
      try {
        ...
        try {
          ...
          console.log('Please start it manually from Services or run "net start MongoDB" as administrator.');
          
          const answer = await askQuestion('Would you like to use MongoDB Atlas instead? (y/n): ');
          if (answer.toLowerCase() === 'y') {
            const uri = await askQuestion('Please enter your MongoDB Atlas connection string: ');
            mongoUri = uri;
            return;
          } else {
            throw new Error('Please start MongoDB service and run this script again.');
          }
        }
      }
      ...
      try {
        if (isLinux) {
          execSync('systemctl status mongodb || systemctl status mongod', { stdio: 'ignore' });
        } else if (isMac) {
          execSync('brew services list | grep mongodb-community', { stdio: 'ignore' });
        }
        console.log('✅ MongoDB service is running.');
      } catch (error) {
        console.log('⚠️ MongoDB service is not running.');
        ...
        try {
          if (isLinux) {
            execSync('sudo systemctl start mongodb || sudo systemctl start mongod', { stdio: 'pipe' });
          } else if (isMac) {
          ...
          if (isLinux) {
            console.log('sudo systemctl start mongodb   OR   sudo systemctl start mongod');
          } else if (isMac) {
    ...
    try {
      execSync(`mongosh --eval "db.adminCommand({ ping: 1 })"`, { stdio: 'ignore' });
      ...
      if (answer.toLowerCase() === 'y') {
        ...
        throw new Error('Please ensure MongoDB is properly installed and running.');
      }
    }
  } catch (error) {
    throw error;
...
async function seedDatabase() {
  ...
  try {
    ...
    throw new Error('Database seeding failed. Please check MongoDB connection and try again.');
  }
}
